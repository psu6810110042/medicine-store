...
export default function PharmacyPage() {
    ...
    const fetchAllOrders = async () => {
        try {
            ...
            const parsedProducts: Product[] = Array.isArray(productsData) ? productsData : ((productsData as { data?: Product[] }).data ?? []);
            const totalStock = parsedProducts.reduce((sum: number, p: Product) => sum + (Number(p.stockQuantity) || 0), 0);
        ...
        <div className="rounded-3xl border bg-white shadow-sm overflow-hidden">
            ...
            <div className="px-5 pb-5 flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {!isFinished && (
                        <>
                            {((order.status === OrderStatus.PROCESSING || order.status === OrderStatus.STOCK) && order.paymentStatus !== "APPROVED") ? (
                                <button
                                    onClick={() => toast.error("ให้แอดมินตรวจสอบสลิปโอนเงินก่อนดำเนินการต่อ")}
                                    className="rounded-xl border bg-slate-100 px-4 py-2 text-sm text-slate-600 hover:bg-slate-200 shadow-sm transition-colors"
