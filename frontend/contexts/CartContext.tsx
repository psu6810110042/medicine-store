...
export const CartProvider = ({ children }: { children: ReactNode }) => {
      ...
      if (user) {
        try {
          ...
          if (res.ok) {
            ...
            const mappedItems = cartData.items.map((item: { product: { id: string }; quantity: number }) => ({
  ...
  const removeItemsLocally = (productIds: string[]) => {
    ...
    if (!user) {
      ...
      const newCart = currentCart.filter((item: { productId: string }) => !productIds.includes(item.productId));
